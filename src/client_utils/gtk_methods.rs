...
fn setup_own_chat_tutorial(buffer: &TextBuffer) {
    let tutorial_text = "🦀 Welcome to Panicked at pensar Nombre's IRC client! 🦀\n
This is your own chat.\n
Apart from the UI, you can also use the following shortcuts:
🔸 /away [message] - Set your away status
🔸 /unaway - Remove your away status
🔸 /whois [nickname] - Get information about a user
🔸 /oper [password] - Become an IRC operator
🔸 /quit [message] - Quit the IRC server
🔸 /squit [server] [comment] - Disconnect a server from the IRC network
🔸 /names - Get a list of users in current channel
🔸 /topic [topic] - Set the topic of the current channel
🔸 /part - Leave current channel
🔸 /invite [nickname] - Invite a user to the current channel
🔸 /kick [nickname] - Kick a user from the current channel
🔸 /mode [mode] - Set the mode of the current channel
🔸 /dcc_chat - Send a DCC chat request to a user
🔸 /dcc_close - Close a DCC chat
Possible modes are:
🔹 +k [key] - Set a channel key
🔹 -k - Remove the channel key
🔹 +l [limit] - Set a user limit
🔹 -l - Remove the user limit
🔹 (+/-)i - Set/remove the channel's invite-only mode
🔹 (+/-)o [nickname] - Give/Remove a user operator status
🔹 (+/-)t - Set/Remove the topic operator-only mode
🔹 (+/-)s - Set/Remove the secret mode
🔹 (+/-)b [nickname] - Ban/Unban a user from the channel
